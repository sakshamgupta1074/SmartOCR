...
def sknw_main_lr(path_black,cor):
	...
	for i in range(1,lengthy):
		if ((abs(final_nodes[i][1] - final_nodes[i-1][1]))<=10):
...
def sknw_loop_lr(cor):
	...
	a=0
	for i in range(1,lengthy):
		if ((final_nodes[i][1] - final_nodes[i-1][1])<=10):
			node_levels[a].append(final_nodes[i])
			node_val[a].append(ps.index(final_nodes[i]))
		else:
			a=a+1
...
# path_original='try2.jpg'

# cor=[[360, 45, 517, 103, 97.1426248550415, 'rectangle'], [360, 144, 517, 203, 96.02874517440796, 'rectangle'], [361, 238, 517, 305, 95.7540512084961, 'rectangle'], [361, 338, 517, 405, 95.51981687545776, 'rectangle'], [22, 178, 194, 251, 85.8630895614624, 'rectangle']]
